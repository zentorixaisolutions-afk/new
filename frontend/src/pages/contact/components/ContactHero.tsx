  ...
  return (
    <section id="hero-section" className="relative min-h-[50svh] md:min-h-[60dvh] flex items-center overflow-hidden noise-overlay">
      <video ref={videoRef} className="absolute inset-0 w-full h-full object-cover object-center lg:object-[62%_center] saturate-[0.9] brightness-[0.85]" autoPlay loop muted playsInline preload="auto" onCanPlay={handleCanPlay}>
        <source src="https://storage.readdy-site.link/project_files/a7669e92-cc43-4f26-805a-e805864048d7/528f10d6-2b70-46c8-800c-3cdcb1d09b22_contact.mp4" type="video/mp4" />
      </video>
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-8 md:px-10 lg:px-14 xl:px-20 pt-24 sm:pt-28 md:pt-36 pb-12 sm:pb-16 md:pb-20">
        <div className="max-w-[min(100%,720px)] lg:max-w-[min(100%,760px)]">
          <Reveal><span className="inline-block text-xs sm:text-xs md:text-sm font-semibold uppercase tracking-[0.12em] sm:tracking-[0.2em] text-accent-300 mb-3 sm:mb-4 md:mb-6">{t('contact.hero.label')}</span></Reveal>
          <Reveal delay={0.1}><h1 className="text-[clamp(1.75rem,9vw,2.5rem)] sm:text-[clamp(2.25rem,8vw,2.875rem)] md:text-[clamp(2.5rem,6.5vw,3.25rem)] lg:text-[clamp(3.25rem,5vw,4.5rem)] xl:text-[clamp(3.75rem,4.5vw,5rem)] font-semibold text-white leading-[1.15] sm:leading-[1.09] md:leading-[1.08] lg:leading-[1.07] xl:leading-[1.06] mb-4 sm:mb-5 md:mb-6 text-balance tracking-tight">{t('contact.hero.title')}</h1></Reveal>
          <Reveal delay={0.2}><p className="text-sm sm:text-sm md:text-base lg:text-lg text-foreground-200/80 max-w-xl leading-relaxed">{t('contact.hero.subtitle')}</p></Reveal>
        </div>
      </div>
